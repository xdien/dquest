...
template <> bool DQField<QStringList>::set(QVariant value) {
  QString str;
...
template <> QVariant DQField<QStringList>::get(bool convert) const {
  QVariant val = DQBaseField::get(convert);

#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
  if (convert && val.typeId() == QMetaType::QStringList) {
#else
...
DQClause DQPrimaryKey::clause() { return DQClause(DQClause::PRIMARY_KEY); }

QString DQPrimaryKey::primaryKeyName() const { return m_primaryKeyName; }
